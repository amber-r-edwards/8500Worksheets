...
evens <- seq(2:8, by=2)
matrix(evens, byrow = TRUE, nrow = 3)
...
#  }
#}
#was immensly overthinking - went over in class 2.19
...
> I deeply overthought it - so even though I had the max() function up front, I overcomplicated how to test it with the TRUE/FALSE. By creating a variable that is equal to the max(), its much easier to just check the variable against the condition of 1990.
...
> This code works because we're indicating the indices of the strings we want it to output - length() means the full length of indices (in this case 1-3 because there are 3 entries). We use it on 'presidents' because it is a string not a value, so there isn't a span of numbers to put (ex: 1:44).
...
```{r}
spring.classes <- c("HIST8500", "HIST8850", "HIST8520")
class.name.function <- function(spring.classes) {
  print(paste("I am enrolled in", spring.classes, sep = " "))
}
#class.name.function()
class.name.function(spring.classes)
```
...
> It printed 3 because - similar to the code block one above - 1 and 2 don't match the criteria, but it printed 4 because it matched the second criteria the loop moved too after next. Next caused it to exit the iteration, but not the loop, whereas after the second critera, break caused it to exit the entire loop. Hence 3 and 4 were the only outputs printed.
...
#  print (i)
#}
#if print "missing" code from statology.org - wouldn't print without it - need it to omit those that don't align with the parameters I set
...
#worked through in class 2.19
#ex - if you want it to print the state names that match the criteria
for (i in 1:nrow(statepopulations)){
    if (is.na(statepopulations$X1800[i])) {
        next
    }
...
```{r}
#for (g in 1:length(gayguides)) {
#  if (g == "Greenville") {
#    print(gayguides$title)
#  }
#}
#not giving error code but also not printing anything

greenville.function <- function(title) {
  for (g in 1:length(gayguides$city)) {
    if (gayguides$city[g] == title) {
      print(gayguides$title[g])
    }
  }
}
greenville.function("Greenville")
```
...
historyStudents <- function(studentname, class) {
  statement <- paste("Hello ", studentname, ". Welcome to ", class, "!", sep=" s")
  statement
}
```
(@) Can you run this function? 
```{r}
historyStudents("Amber", "HIST8500")
```
...
```{r}
character.counter <- function(string) {
  count <- nchar(string)
  print(paste("There are",count, "characters in that string.", sep = " "))
}
character.counter("pls work")
```

(@) Reflect on the process of building the above function. How did you go about figuring this out? 

>I knew I needed to set up the function with the argument as the string to be examined, and I knew that I would need the character value to be made into a temporary variable so I could paste it into the return value. I first looked up what function counted the number of characters in a string, to which i found nchar(), which I used to align with the argument to make the temporary variable "count." Then I pasted everything together with the rest of the sentence for the result value.
...
>George Washington's grade doesn't get printed be ause in this string you're just assigning the variable George Washington it's grade, you're not restating it to print the actual grade output.
...
```{r}
gg.state.locations <- function(d.state, d.year) {
  for (i in 1:length(gayguides$state)) {
    if (gayguides$state[i] == d.state && gayguides$Year[i] == d.year) {
      print(paste("Found a location in", gayguides$state[i], "called", gayguides$title[i], ".", sep = " "))
    }
  }
}
gg.state.locations("NY", 1979)
gg.state.locations("KY", 1980)
```
...
```{r}
occupation.years <- function(job) {
  for (i in 1:length(BostonWomenVoters$Occupation)) {
    if (BostonWomenVoters$Occupation[i] == job) {
      print(BostonWomenVoters$Name[i])
    }
  }
}
occupation.years("Housewife")
```
