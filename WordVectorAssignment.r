...
#Results: 
# - vol1PCA - identifiable split but larger cluster/number of words toward middle and near to white; nearest to black (poor, fathers, company, significant), cluster(southern, catholic), cluster(history, organizing), dates and places; nearest to white (supremacy, worker, collar, heterosexual), cluster(professional, capitalism, middle, obvious, educated?), cluster(minority, labor, represent, class, ruling, employment), cluser(?, industrial, families, husbands, conflict, partiarchy, creates), cluster(oriented, standards, jobs, ?)
# - vol5PCA - one main cluster near the middle, white and black as outliers but can still see some distribution up and down; nearer to white(middle, african, banned, threatened), cluster(solipsism, superiority, colored, mainly, objects, oppressed); tightly bound cluster in middle, can identify (police, racist, man, marry, racial, identity); nearer to black(attempted), cluster(excuse, slave, brownmiller's, reaction, implication, slaves), cluster(rapists, convicted, crime, ?, victims, sourhtern)

#vol1
vol1race <- vol1model[[c("black", "white"), average = F]]
common_similarities_vol1race <- vol1model[1:2875, ] %>% cosineSimilarity(vol1race)
common_similarities_vol1race[20:30, ]
high_similarities_vol1race <- common_similarities_vol1race[rank(-apply(common_similarities_vol1race, 1, max)) < 75, ]
high_similarities_vol1race %>%
    prcomp() %>%
    biplot(main = "50 words in a projection of black and white")

#vol5
vol5race <- vol5model[[c("black", "white"), average = F]]
common_similarities_vol5race <- vol5model[1:3133, ] %>% cosineSimilarity(vol5race)
common_similarities_vol5race[20:30, ]
high_similarities_vol5race <- common_similarities_vol5race[rank(-apply(common_similarities_vol5race, 1, max)) < 75, ]
high_similarities_vol5race %>%
    prcomp() %>%
    biplot(main = "50 words in a projection of black and white")

#almost opposite shapes/patterns
...
#   After completing my plots for question 2, I decided to use the models I created for volume 1 and 5 to run the same parameters around race to see what patterns emerge. The spatial visualizations take two almost inverted shapes (plots uploaded as PNGs on github)and seem to suggest a more unique split between usage "white" and "black" in volume 1 whereas volume 5 discussed "white" and "black" in a more uniform way. Words nearest to "black" in volume 1 centered around poverty, religion, the South, and potentially the history of organizing - aligning with previously discussed early discourse of feminist activism linked to antiracist activism. Nearest to "white" are words regarding supremacy and conflict as well as education, work, and the home gesturing toward more theorhetical discussions of the structures of American society. The visualization of volume 5 seems to indicate that there are not many words spatially near "white" or "black" with one tight cluster right in the middle and then some slight dispersion in the direction of one or the other. This alone indicates to me either that the words are being used in very similar ways such as in a more in depth discussion of racism, or that there is less discussion of race in multiple contexts as seen in volume 1. The words slightly nearer to "white" and "black" align with what was found in volume 1 except for the presence of slavery and interracial marriage. Altogether, these PCA embeddings indicate that feminist discourse in Quest certainly changed over time, but do not directly suggest emergent separate feminisms - potentially a question of authorship/contributors and the perspectives they may (or may not) be providing.
